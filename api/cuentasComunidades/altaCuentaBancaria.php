<?php 
  ...
  if (is_array($params['id_asociativo_banco'])) {
    $id_banco = implode("','", $params['id_asociativo_banco']);
  } else {
    $id_banco = $params['id_asociativo_banco'];
  }

  // REALIZA LA QUERY A LA DB
  $query = mysqli_query($conexion, "SELECT banco.id_banco 
                                    FROM   banco
                                    WHERE  banco.id_asociativo_banco='$id_banco'");
...
if ($resultadoQuery = mysqli_fetch_array($query))  
{
  ...
  $id_asociativo_banco = implode("','", $id_banco_array);
  ...
  $id_comunidad = $params['id_comunidad'];
